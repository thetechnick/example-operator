...
MODULE:=github.com/thetechnick/example-operator
...
API_BASE:=example.thetechnick.ninja
export SKIP_TEARDOWN?=

# Container
IMAGE_ORG?=quay.io/nschiede
example_OPERATOR_MANAGER_IMAGE?=$(IMAGE_ORG)/example-operator-manager:$(VERSION)
...
all: \
	bin/linux_amd64/example-operator-manager
...
run-example-operator-manager: generate fmt vet manifests
	go run -ldflags "-w $(LD_FLAGS)" \
		./cmd/example-operator-manager/main.go \
			-metrics-addr=""
.PHONY: run-example-operator-manager
...
manifests: $(CONTROLLER_GEN)
	...
	@controller-gen crd:crdVersions=v1,generateEmbeddedObjectMeta=true \
		rbac:roleName=example-operator-manager \
		paths="./..." \
...
# Run the E2E testsuite after installing the exampleOperator into the cluster.
...
load-example-operator: build-image-example-operator-manager
	@source hack/determine-container-runtime.sh \
		&& $$KIND_COMMAND load image-archive \
			.cache/image/example-operator-manager.tar \
			--name example-operator-e2e
.PHONY: load-example-operator

# Template deployment
config/deploy/deployment.yaml: FORCE $(YQ)
	@yq eval '.spec.template.spec.containers[0].image = "$(example_OPERATOR_MANAGER_IMAGE)"' \
			config/deploy/deployment.yaml.tpl > config/deploy/deployment.yaml

# Installs the example Operator into the kind e2e cluster.
apply-example-operator: $(YQ) load-example-operator config/deploy/deployment.yaml
	@echo "installing example Operator $(VERSION)..."
	@(source hack/determine-container-runtime.sh \
		&& kubectl apply -f config/deploy \
		&& echo -e "\nwaiting for deployment/example-operator..." \
		&& kubectl wait --for=condition=available deployment/example-operator -n example-operator --timeout=240s \
		&& echo) 2>&1 | sed 's/^/  /'
.PHONY: apply-example-operator
...
build-images: \
	build-image-example-operator-manager
.PHONY: build-images

push-images: \
	push-image-example-operator-manager
.PHONY: push-images
